...
def print_report(result):
    ...
    print(f"\n{SEP2}")
    print(f"  RBC — Risk Bridge Capital | 0DTE Scanner v1.4-beta")
    print(f"  {result['date']}")
    print(f"{SEP2}")
...
def main():
    ...
    print("\n" + "═" * 60)
    print("  RBC — Risk Bridge Capital | 0DTE Scanner v1.4-beta")
    print(f"  {now_et}")
    print("═" * 60)
    print("""
  MODOS DE USO:
    ...
    # ── Dados SpotGamma — comuns a todos os modos ───────────────────
    print("\n  Cole os dados do SpotGamma (Enter para usar exemplo de hoje):")
    ...
    # ── Menthor Q — opcional em todos os modos ───────────────────────
    print("\n  Cole os dados do Menthor Q (Enter para usar exemplo de hoje):")
    raw_mq = input("  > ").strip()
    if not raw_mq:
        raw_mq = (
            "$SPY: Call Resistance, 745, Put Support, 730, HVL, 739, "
            "1D Min, 732.47, 1D Max, 745.87, Call Resistance 0DTE, 750, "
            "Put Support 0DTE, 730, HVL 0DTE, 748, Gamma Wall 0DTE, 750, "
            "GEX 1, 740, GEX 2, 738, GEX 3, 735, GEX 4, 733, GEX 5, 737, "
            "GEX 6, 728, GEX 7, 732, GEX 8, 751, GEX 9, 746, GEX 10, 727"
        )
        print("  (usando dados Menthor Q de exemplo)")
    mq_data = parse_mq_data(raw_mq)
    mq_spy  = mq_data.get('SPY', {})

    # ── MODO 1: Pré-mercado ─────────────────────────────────────────
    if modo == '1':
        print("\n" + "─" * 60)
        print("  PRÉ-MERCADO — Níveis do dia carregados")
        print("─" * 60)
        if spy_ref:
            d = spy_ref
            print(f"\n  ── SpotGamma ──")
            print(f"  Call Wall        : {d['call_wall']}")
        ...
        if mq_spy:
            m = mq_spy
            print(f"\n  ── Menthor Q ──")
            print(f"  Range do dia     : {m.get('range_min')} – {m.get('range_max')}")
            print(f"  HVL              : {m.get('hvl')}  |  HVL 0DTE: {m.get('hvl_0dte')}")
            print(f"  Call Res 0DTE    : {m.get('call_res_0dte')}  |  Put Sup 0DTE: {m.get('put_sup_0dte')}")
            print(f"  Gamma Wall 0DTE  : {m.get('gamma_wall_0dte')}")
            print(f"  GEX Top 5        : {m.get('gex_strikes', [])[:5]}")
        print(f"\n  ★ Níveis mais importantes hoje:")
        cw = spy_ref.get('call_wall') if spy_ref else None
        gw = mq_spy.get('gamma_wall_0dte') if mq_spy else None
        if cw and gw:
            print(f"    Resistência: SG {cw} | MQ Gamma Wall {gw} {'✅ confirmados' if abs(cw-gw)<=2 else '⚠ divergência'}")
        pw = spy_ref.get('put_wall') if spy_ref else None
        ps = mq_spy.get('put_sup_0dte') if mq_spy else None
        if pw and ps:
            print(f"    Suporte    : SG {pw} | MQ Put Sup 0DTE {ps} {'✅ confirmados' if abs(pw-ps)<=3 else '⚠ divergência'}")
        if mq_spy:
            print(f"    Range      : {mq_spy.get('range_min')} – {mq_spy.get('range_max')} "
                  f"(amplitude {(mq_spy.get('range_max',0)-mq_spy.get('range_min',0)):.2f} pts)")
        print(f"\n  Ação: Rode o modo 2 às 9:35 com VIX e HIRO.")
        return

    # ── MODO 2: Abertura (primeiros 30 min) ─────────────────────────
    if modo == '2':
        print("\n  Dados de abertura (TradingView + Barchart):")
        try:
            vix_open  = float(input("  VIX na abertura (9:30)      : "))
            vix_now   = float(input("  VIX agora                    : "))
            spot_open = float(input("  SPY preço abertura (9:30)    : "))
            spot_now  = float(input("  SPY preço agora              : "))

            print("\n  Dados Barchart — linha 0DTE (Enter para pular):")
            print("  ⚠  Volume: digite o número inteiro. Ex: 1200000 ou 1,200,000. Não use 1.2M.")
            pc_now_s  = input("  Put/Call ratio agora         : ").strip()
            pc_prev_s = input("  Put/Call ratio há 15 min     : ").strip()
            cv_s      = input("  Call volume 0DTE (inteiro)   : ").strip().replace(',','')
            pv_s      = input("  Put  volume 0DTE (inteiro)   : ").strip().replace(',','')

        except ValueError:
            print("  Erro de input. Use valores numéricos.")
            return

        # ── Calcula HIRO sintético se tiver dados do Barchart ────────
        hiro = 'neutral'
        if pc_now_s and pc_prev_s:
            try:
                pc_now  = float(pc_now_s)
                pc_prev = float(pc_prev_s)
                cv      = int(cv_s) if cv_s else 0
                pv      = int(pv_s) if pv_s else 0
                above_zg = spy_ref and spot_now > spy_ref.get('zero_gamma', 0)
                above_vt = spy_ref and spot_now > spy_ref.get('vol_trigger', 0)

                # Importa função do HIRO sintético inline
                score_h = 0
                # P/C ratio nível
                if pc_now < 0.70:   score_h += 3
                elif pc_now < 0.85: score_h += 2
                elif pc_now < 1.00: score_h += 1
                elif pc_now < 1.20: score_h -= 1
                else:               score_h -= 3
                # P/C direção
                pc_delta = pc_now - pc_prev
                if pc_delta <= -0.10:   score_h += 2
                elif pc_delta <= -0.03: score_h += 1
                elif pc_delta >= 0.10:  score_h -= 2
                elif pc_delta >= 0.03:  score_h -= 1
                # Volume
                if cv and pv:
                    call_pct = cv / (cv + pv) * 100
                    if call_pct >= 60:   score_h += 2
                    elif call_pct >= 52: score_h += 1
                    elif call_pct < 45:  score_h -= 2
                # Spot + VIX
                vix_d = vix_now - vix_open
                spot_d = spot_now - spot_open
                if spot_d > 0.5 and vix_d < 0:    score_h += 2
                elif spot_d < -0.5 and vix_d > 0:  score_h -= 2
                elif spot_d > 0.5 and vix_d > 0.5: score_h -= 1
                # Contexto SG
                if above_zg and above_vt: score_h += 1
                elif not above_zg:         score_h -= 1

                if score_h >= 4:    hiro = 'positive'
                elif score_h <= -3: hiro = 'negative'
                else:               hiro = 'neutral'

                print(f"\n  HIRO sintético calculado: {hiro.upper()}  (score: {score_h:+d})")
                print(f"  P/C: {pc_prev:.2f} → {pc_now:.2f} ({pc_delta:+.2f}) | "
                      f"SPY: {spot_d:+.2f} | VIX: {vix_d:+.2f}")
                if cv and pv:
                    print(f"  Calls: {cv:,} ({cv/(cv+pv)*100:.0f}%) | Puts: {pv:,} ({pv/(cv+pv)*100:.0f}%)")
            except Exception:
                print("  Não foi possível calcular o HIRO sintético — usando neutral.")
                hiro = 'neutral'
        else:
            # Sem dados Barchart — pede direção manual
            hiro_raw = input("\n  HIRO direction [positive/negative/neutral]: ").strip().lower()
            hiro = hiro_raw if hiro_raw in ('positive','negative','neutral') else 'neutral'

        # Passa mq_data para integrar MQ no score do veredicto
